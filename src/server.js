...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dabotlist.ml/user/${a.id}) logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "You have been blocked from DaBotList.",
                    logout: true
                })
                req.logout();
            });
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://dabotlist.ml/user/${a.id}) Just Logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://dabotlist.ml/`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://dabotlist.ml/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://dabotlist.ml/bot/" + bot.botID + "</loc></url>";
        })
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}]https://dabotlist.ml/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
            .addField("Code Link", `https://dabotlist.tk/code/${kod}`, true)
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://dabotlist.ml/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addField("Code Link", `https://dabotlist.ml/code/${kod}`, true)
            .addField("Code Description", rBody['codedesc'], true)
            .addField("Code Category", rBody['category'], true)
        )
        res.redirect('/code/' + kod)
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> Topic has been switched to __maintance__ due to **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("DaBotList.xyz", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`<a:online:833375738785824788> DaBotList are **active** again!\n[Click to redirect website](https://dabotlist.ml)`)
            .setFooter("DaBotList © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~DaBotList has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
